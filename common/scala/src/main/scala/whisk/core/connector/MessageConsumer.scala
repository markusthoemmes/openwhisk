...
object MessageFeed {
  sealed trait FeedState
  case object Idle extends FeedState
  case object FillingPipeline extends FeedState
  case object DrainingPipeline extends FeedState

  sealed trait FeedData
  case object NoData extends FeedData
  ...
  case class FillCompleted(messages: Seq[(String, Int, Long, Array[Byte])])
}
...
class BatchingMessageFeed(description: String,
                          logging: Logging,
                          consumer: MessageConsumer,
                          maximumHandlerCapacity: Int,
                          longPollDuration: FiniteDuration,
                          handler: immutable.Queue[Array[Byte]] => Unit,
                          autoStart: Boolean = true,
                          logHandoff: Boolean = true)
    extends FSM[MessageFeed.FeedState, MessageFeed.FeedData] {
  import MessageFeed._
  ...
  private def sendOutstandingMessages(): Unit = {
    ...
    if (occupancy > 0 && handlerCapacity > 0) {
      ...
      val send = handlerCapacity min occupancy
      val (toSend, rest) = outstandingMessages.splitAt(send)
      outstandingMessages = rest

      handler(toSend.map(_._4))
      handlerCapacity -= send
